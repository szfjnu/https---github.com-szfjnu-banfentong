...
async function checkPermission(openid, allowedRoles) {
  ...
  if (!user) {
    const relRes = await db.collection('user_class_relation').where({ user_openid: openid }).limit(1).get()
    ...
    if (relRes.data && relRes.data.length > 0 && relRes.data[0].role) {
      user = { role: relRes.data[0].role, _openid: openid }
    }
  }

  if (!user) {
    const relRes2 = await db.collection('user_class_relation').where({ _openid: openid }).limit(1).get()
    ...
    if (relRes2.data && relRes2.data.length > 0 && relRes2.data[0].role) {
      user = { role: relRes2.data[0].role, _openid: openid }
    }
  }
