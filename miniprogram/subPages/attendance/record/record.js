...
Page({
  data: {
    ...
    // 编辑模式相关
    editStudents: [], // 待编辑的学生列表
    selectedCategoryId: '', // 选中的考勤类别
    selectedCategoryName: '',
    selectedCategoryCode: '', // 选中的考勤类别代码
  ...
  loadStudents: async function () {
    try {
      ...
      const students = [{ student_id: '', name: '全部学生' }, ...(res.data || [])];
      this.setData({ 
        students: students,
  ...
  loadEditStudents: async function () {
    try {
      ...
      // 组装学生列表
      const editStudents = (res.data || []).map(student => ({
        ...student,
        attendance: attendanceMap[student.student_id] || null,
        selected: false
      }));
      
      this.setData({ editStudents: editStudents });
  ...
  onSelectCategory: function (e) {
    ...
    const categoryCode = e.currentTarget.dataset.code || categoryId; // 获取category_code
    this.setData({
  ...
  onSelectStudent: function (e) {
    ...
    const student = this.data.editStudents[index];
    const editStudents = this.data.editStudents;
    editStudents[index].selected = !student.selected;
    this.setData({ editStudents: editStudents });
  },

  // 编辑模式：全选
  onSelectAll: function () {
    const editStudents = this.data.editStudents.map(s => ({ ...s, selected: true }));
    this.setData({ editStudents: editStudents });
  },

  // 编辑模式：保存考勤记录
  onSaveAttendance: async function () {
    const { selectedCategoryId, selectedCategoryCode, selectedDate, editStudents } = this.data;
    ...
    const selectedStudents = editStudents.filter(s => s.selected);
