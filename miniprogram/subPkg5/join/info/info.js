...
Page({
  ...
  onNext: function () {
    ...
    if (this.data.selectedRole === 'student') {
      ...
      wx.navigateTo({
        url: `/subPkg5/join/confirm/confirm?classId=${this.data.classId}&role=student&addNew=true`
      });
    } else if (this.data.selectedRole === 'parent') {
      // 家长身份，跳转到学生绑定页
      wx.navigateTo({
        url: `/subPkg5/join/bind/bind?classId=${this.data.classId}`
      });
    } else {
      // 老师身份，直接跳转到确认页
      wx.navigateTo({
        url: `/subPkg5/join/confirm/confirm?classId=${this.data.classId}&role=teacher`
      });
    }
  }
});
